...
import math			#Necesaria para calcular distancia euclediana
import operator		#Necesaria para tomar los valores de un arreglo en una posicion exacta
...
def loadDataset(filename, split, trainingSet=[] , testSet=[]):
	with open(filename, 'r') as csvfile:                        #Se abre el archivo
	    ...
	    dataset = list(lines)									#Se le asignan las lineas del dataset a la variable dataset
	    for x in range(len(dataset)):							#Se itera por el total de lineas del dataset
	        for y in range(12):                                 #Itera varia dependiendo del numero de atributos del dataset
...
def euclideanDistance(instance1, instance2, length):
	...
	return math.sqrt(distance)									#Raiz cuadrada de la suma del cuadrado de la diferencia entre dos numeros
...
def getNeighbors(trainingSet, testInstance, k):
	...
	for x in range(len(trainingSet)):									#Itera por el total de lineas en el set de entrenamiento
...
#Método usado para elegir la clasificación con mas 'votos' entre los vecinos
def getResponse(neighbors):
	...
	for x in range(len(neighbors)):										#Itera dependiendo del total de vecinos
		...
		if response in classVotes:										#Hace un conteo de los votos de cada clasificacion de los vecinos
			classVotes[response] += 1									
		else:
			classVotes[response] = 1
	sortedVotes = sorted(classVotes.items(), key=operator.itemgetter(1), reverse=True) #Ordena los votos de forma descendente
	return sortedVotes[0][0]											#Regresa la clasificiacion con mas votos/la primera del diccionario
...
def getAccuracy(testSet, predictions):
	...
	for x in range(len(testSet)):										#Itera todo el set de prueba
		if testSet[x][-1] == predictions[x]:							#Si la clasificion de la instancia es igual a la predicción
			correct += 1												#Se aumentan las predicciones correcta
